...
def get_executor(config, model, data_feature):
    ...
    """
    try:
        return getattr(importlib.import_module('exec.executor'),
...
def get_model(config, data_feature):
    ...
    if config['task'] == 'traj_loc_pred':
        try:
            return getattr(importlib.import_module('model.trajectory_loc_prediction.DeepMove'),
...
def get_evaluator(config):
    ...
    """
    try:
        return getattr(importlib.import_module('eval.evaluator'),
